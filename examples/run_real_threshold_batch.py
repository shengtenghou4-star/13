...
OUTPUT_ROOT = Path("outputs/real-threshold-batch")
DEPTHS = (0.0005, 0.001, 0.002)
...
for row in rows:
    ...
    try:
        ...
        null_screen, dimmings, brightenings, trials, cells = run_real_lightcurve_campaign(
            lc,
            depths=DEPTHS,
...
summary = {
    "experiment": "HOU-EARTH shallow real TESS threshold sweep",
    "status": "calibration; not a discovery search",
    "depths": DEPTHS,
    "durations_days": DURATIONS_DAYS,
    "seeds": list(SEEDS),
    "planned_trials": len(rows) * len(DEPTHS) * len(DURATIONS_DAYS) * len(SEEDS),
...
if not all_trials:
    raise SystemExit("No real TESS threshold target completed")
