...
def surrogates(
    ...
    *,
    method: str = GAP_AWARE_METHOD,
    neutralized_events: int = 0,
) -> list[SurrogateTrial]:
...
def build(
    dimming: list[SingleTransitEvent],
    controls: list[SingleTransitEvent],
    nulls: list[SurrogateTrial] | None = None,
):
    return build_blind_candidate_inputs(
        ...
        surrogate_trials=surrogates() if nulls is None else nulls,
    )
...
def test_real_event_rows_use_add_one_familywise_p_and_matched_control() -> None:
    ...
    rows, receipt = build(
        ...
        [
            event(2.0, 0.08, 8.0, direction="brightening"),
            event(3.0, 0.08, 9.0, direction="brightening"),
            event(4.0, 0.16, 20.0, direction="brightening"),
        ],
        surrogates(maxima),
    )
    assert len(rows) == 1
...
def test_machine_event_order_and_indices_are_input_order_invariant() -> None:
    events = [
        event(9.0, 0.16, 8.0, direction="dimming"),
        event(4.0, 0.08, 10.0, direction="dimming"),
    ]
...
def test_nearest_duration_control_is_selected_deterministically() -> None:
    rows, _ = build(
        [event(8.0, 0.116, 10.0, direction="dimming")],
        [
            event(2.0, 0.104, 7.0, direction="brightening"),
            event(3.0, 0.16, 30.0, direction="brightening"),
        ],
    )
...
def test_event_target_direction_and_duration_family_are_closed() -> None:
    wrong_target = SingleTransitEvent(
        target="OTHER STAR",
...
def test_campaign_input_hash_must_come_from_fingerprinted_arrays() -> None:
    ...
    lightcurve = LightCurve(
        ...
        np.ones_like(time),
        target="SYNTHETIC STAR",
        metadata={"campaign_input_array_hashes": {"combined_sha256": HASH}},
    )
