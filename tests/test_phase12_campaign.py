...
SOURCE_COMMIT = "e" * 40
FROZEN_AT = "2026-07-23T11:20:00Z"
...
def downloader(query: str, **_: object) -> LightCurve:
    time = np.linspace(0.0, 48.0, 1800)
    return LightCurve(
        time,
        np.ones_like(time),
...
def searcher(
    lc: LightCurve, *, direction: str, **_: object
...
def test_phase12_campaign_freezes_sixty_four_target_private_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert receipt["minimum_products_per_target"] == 2
    assert receipt["minimum_distinct_sectors_per_target"] == 2
    ...
    assert (sink / "phase12_selection_lock.json").is_file()
    assert (sink / "batch_plan.json").is_file()
    assert (sink / "catalog_audit" / "nasa_ps_snapshot.csv").is_file()
    assert len(list((sink / "campaign_inputs").glob("batch-*/*.csv"))) == 64
