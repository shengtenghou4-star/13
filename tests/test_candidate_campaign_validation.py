...
def package() -> dict[str, object]:
    ...
    machine_rows, receipt = build_blind_candidate_inputs(
        ...
        surrogate_trials=surrogate_rows,
    )
    ...
    lock_payload = {
        ...
        "targets": [
            {
                "target_id": "synthetic-star",
                "query": "SYNTHETIC STAR",
                "intended_role": "synthetic-null",
                "sector_label": "1",
...
def test_rehashed_surrogate_change_cannot_fake_event_p_value() -> None:
    ...
    payload["target_calibrations"][0]["surrogate_trials"][0][
        "maximum_dimming_snr"
    ] = 20.0
...
def test_plain_campaign_tampering_breaks_package_hash() -> None:
    ...
    with pytest.raises(CandidateCampaignValidationError) as captured:
        validate_candidate_campaign_evidence(payload)
    assert any(
        "package SHA-256" in error for error in captured.value.report.errors
    )


def test_rehashed_campaign_lock_human_note_is_rejected() -> None:
    payload = deepcopy(package())
    payload["campaign_lock"]["human_note"] = "open this one first"
    lock = payload["campaign_lock"]
    lock["campaign_lock_sha256"] = canonical_json_sha256(
        {key: value for key, value in lock.items() if key != "campaign_lock_sha256"}
    )
    assert_rejected(payload, "campaign lock contains undeclared fields: human_note")


def test_rehashed_lock_target_and_receipt_notes_are_rejected() -> None:
    payload = deepcopy(package())
    payload["campaign_lock"]["targets"][0]["manual_score"] = 99
    lock = payload["campaign_lock"]
    lock["campaign_lock_sha256"] = canonical_json_sha256(
        {key: value for key, value in lock.items() if key != "campaign_lock_sha256"}
    )
    assert_rejected(payload, "contains undeclared fields: manual_score")

    payload = deepcopy(package())
    payload["target_calibrations"][0]["calibration_receipt"]["human_note"] = "great"
    assert_rejected(payload, "calibration receipt contains undeclared fields: human_note")


def test_rehashed_lock_threshold_or_provenance_drift_is_rejected() -> None:
    payload = deepcopy(package())
    payload["campaign_lock"]["minimum_search_snr"] = 4.0
    lock = payload["campaign_lock"]
    lock["campaign_lock_sha256"] = canonical_json_sha256(
        {key: value for key, value in lock.items() if key != "campaign_lock_sha256"}
    )
    assert_rejected(payload, "minimum search SNR is inconsistent")

    payload = deepcopy(package())
    payload["campaign_lock"]["targets"][0]["query_provenance_sha256"] = "bad"
    lock = payload["campaign_lock"]
    lock["campaign_lock_sha256"] = canonical_json_sha256(
        {key: value for key, value in lock.items() if key != "campaign_lock_sha256"}
    )
    assert_rejected(payload, "query_provenance_sha256 is invalid")


def test_rehashed_surrogate_count_flag_inconsistency_is_rejected() -> None:
    payload = deepcopy(package())
    trial = payload["target_calibrations"][0]["surrogate_trials"][0]
    trial["maximum_brightening_snr"] = 6.0
    trial["exceeded_brightening_threshold"] = True
    trial["brightening_events"] = 0
    assert_rejected(payload, "surrogate trial 0 is invalid")
