...
def trial(target: str, recovered: bool, snr: float | None) -> RealInjectionTrial:
...
def test_pool_real_trials_counts_targets_and_interval() -> None:
    cells = pool_real_trials(
        [
            trial("star-a", True, 9.0),
            trial("star-a", False, None),
            trial("star-b", True, 7.0),
            trial("star-b", True, 8.0),
        ]
    )
    assert len(cells) == 1
    cell = cells[0]
