...
def validate_phase07_summary(
    ...
    *,
    ...
    if not isinstance(targets_value, Sequence) or isinstance(targets_value, (str, bytes)):
        targets: list[Mapping[str, object]] = []
    else:
        targets = [
            item for item in targets_value if isinstance(item, Mapping)
        ]
    ...
    completed_null = [
        ...
        and item.get("surrogate_summary", {}).get("status") == "completed"
    ]
    physical_trials = sum(int(item.get("physical_trials", 0)) for item in completed)
    surrogate_trials = sum(int(item.get("surrogate_trials", 0)) for item in completed)
    errors: list[str] = []
    ...
    for item in completed:
        ...
        count = int(item.get("physical_trials", 0))
        ...
        surrogate_count = int(item.get("surrogate_trials", 0))
        surrogate_status = item.get("surrogate_summary", {}).get("status")
    ...
    reported_physical = int(summary.get("total_physical_trials", physical_trials))
    reported_surrogate = int(summary.get("total_surrogate_trials", surrogate_trials))
    ...
    minimum_p = summary.get("minimum_resolvable_surrogate_p")
    expected_p = 1.0 / (surrogate_trials_per_null_target + 1.0)
    if minimum_p is None or abs(float(minimum_p) - expected_p) > 1e-12:
        errors.append(
