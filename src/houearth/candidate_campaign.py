...
def _canonical_event_key(event: SingleTransitEvent) -> tuple[object, ...]:
    return (
        ...
        -float(event.snr),
        -float(event.depth),
        int(event.local_points),
        event.direction,
    )


def _validate_surrogate_trials(
    trials: Iterable[SurrogateTrial],
    *,
    target_name: str,
    sector_label: str,
) -> tuple[SurrogateTrial, ...]:
...
def _matched_brightening_control(
    event: SingleTransitEvent,
    ...
    duration = min(
        available,
        key=lambda value: (abs(value - float(event.duration_days)), value),
    )
    values = [
        float(control.snr)
        for control in controls
        if math.isclose(
            float(control.duration_days), duration, rel_tol=0.0, abs_tol=1e-12
        )
    ]
    return duration, max(values)


def build_blind_candidate_inputs(
    *,
    target_id: str,
    target_name: str,
    sector_label: str,
    campaign_input_sha256: str,
    search_duration_family_days: Sequence[float],
    dimming_events: Iterable[SingleTransitEvent],
    brightening_control_events: Iterable[SingleTransitEvent],
    surrogate_trials: Iterable[SurrogateTrial],
) -> tuple[list[BlindCandidateInput], CandidateCalibrationReceipt]:
    ...
    if (
        not isinstance(campaign_input_sha256, str)
        or _SHA256_PATTERN.fullmatch(campaign_input_sha256) is None
    ):
    ...
    for event in dimming:
        _validate_event(
            event, target_name=target_name, direction="dimming", durations=durations
        )
    for event in controls:
        _validate_event(
            event, target_name=target_name, direction="brightening", durations=durations
        )
    dimming = tuple(sorted(dimming, key=_canonical_event_key))
    controls = tuple(sorted(controls, key=_canonical_event_key))

    null_trials = _validate_surrogate_trials(
        surrogate_trials,
    ...
    maxima = [
        None
        if trial.maximum_dimming_snr is None
        else float(trial.maximum_dimming_snr)
    ...
    for source_index, event in enumerate(dimming):
        ...
        rows.append(
            BlindCandidateInput(
                ...
                snr=float(event.snr),
                empirical_familywise_p=float(p_value),
                matched_brightening_snr=control_snr,
                snr_above_matched_control=margin,
                campaign_input_combined_sha256=campaign_input_sha256,
                search_duration_family_days=durations,
                source_event_index=source_index,
                event_direction="dimming",
            )
        )

    receipt = CandidateCalibrationReceipt(
        schema=PHASE09_CALIBRATION_SCHEMA,
        target_id=target_id,
        target_name=target_name,
        sector_label=sector_label,
        campaign_input_combined_sha256=campaign_input_sha256,
        ...
        surrogate_trials=len(null_trials),
        surrogate_trials_without_dimming_maximum=sum(
            maximum is None for maximum in maxima
        ),
        minimum_resolvable_familywise_p=1.0 / denominator,
...
def freeze_candidate_campaign_evidence(
    *,
    source_commit: str,
    frozen_at_utc: str,
    campaign_lock: Mapping[str, object],
    target_calibrations: Sequence[Mapping[str, object]],
